...
test('healthcheck is negative if the engine is unreachable', async () => {
  expect.assertions(3);
  ...
  try {
    ...
    expect(response.data.engine.last_successful_connection).toBe(null);
    expect(response.data.healthcheck).toBe('UP');
  }
});
...
test('healthcheck is will never get green if the scanner is not configured to use basic-auth for a secured engine', async () => {
  expect.assertions(3);
  taskApi = new MockTaskApi();
...
test('healthcheck is will never get green if the scanner has the wrong basic-auth creds', async () => {
  expect.assertions(3);
  taskApi = new MockTaskApi();
  await taskApi.startServer(jobApiPort++, { secured: true });
