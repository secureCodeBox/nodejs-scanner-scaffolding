...
function fetchJob(engineAddress, topicName, scannerId, requestOptions) {
    return axios
        .post(`${engineAddress}/box/jobs/lock/${topicName}/${scannerId}`, {}, requestOptions)
...
function submitFailure(engineAddress, scannerId, scannerType, jobId, error, requestOptions) {
    if (isString(error)) {
